...
class TrainerBase:
    ...
    def _on_train_epoch(self):
        ...
        if self.ema is not None and self.epoch != self.epoch_start:  # use normal parameters for training, not EMA model
        ...
        for cur_step, inputs in enumerate(dataloader):
            ...
            with autocast(enabled=self.enable_amp):
                ...
                loss_meta = self.loss_func(outputs)
            # backward
            if isinstance(loss_meta, dict):
                loss = sum([v[0] for _, v in loss_meta.items()])
            else:
                loss = loss_meta
            ...
            niter = self.epoch * len(dataloader) + cur_step
            if self.ema is not None:
                self.ema(self.model, niter)
        ...
        self.meter.reset()
        if self.ema is not None:
            self.ema.assign(self.model)  # EMA model
